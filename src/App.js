...
/* ===========================================================
   OUT-OF-THIS-WORLD • HYPER ENHANCED PORTFOLIO (saffron-first)
   - Default theme: SAFFRON
   - Default mode: DARK
   - Removed color changer & theme editor/palette
   - Mobile UX overhaul + bottom nav
   - Project cards use saffron accents (no per-repo hues)
   =========================================================== */
...
// ------------------------------
// MOBILE NAV (bottom, visible improvement)
// ------------------------------
function MobileNav({ active, onJump, colors }) {
  return (
    <nav className="fixed bottom-4 left-1/2 -translate-x-1/2 z-40 w-[94%] md:hidden">
      <div className="flex justify-around items-center rounded-2xl px-3 py-2 border border-white/10 backdrop-blur-xl"
           style={{ background: "rgba(0,0,0,0.45)", boxShadow: `0 10px 30px ${colors.primary}22` }}>
        {navItems.map(({ id, label }) => {
          const is = active === id;
          return (
            <button key={id} onClick={() => onJump(id)} className="flex-1 py-2 mx-1 rounded-xl text-xs font-semibold"
              style={{
...
export default function App() {
  ...
  // Force defaults: saffron + dark mode
  const colors = baseThemes.saffron;
    ...
    () => [
      {
        ...
        description:
          "End-to-end React + FastAPI app that delivers stock data, news, and AI insights for smarter investment decisions.",
      ...
      },
      {
        title: "StockVision",
        description:
          "Predicts future stock prices from real market data with interactive charts, favorites, and dark mode.",
      ...
      },
      {
        title: "MapleLoom",
        description:
          "Private RAG interface powered by Ollama, Qdrant, Meilisearch — offline, source-anchored answers.",
  ...
  return (
    <>
      ...
      {/* Custom DOM cursor ring (hidden on touch devices) */}
      <CursorRing />
      ...
      <div className={`relative bg-black/70 dark:bg-gray-950 text-gray-100 min-h-screen transition-colors duration-700 font-sans`} style={{ paddingBottom: "110px" }}>
        ...
        {/* Scroll progress (subtle on mobile) */}
        ...
        <nav className={`fixed top-0 w-full z-40 backdrop-blur-md bg-black/35 border-b border-white/10 transition-shadow ${scrolled ? "shadow-lg" : ""}`} role="navigation" aria-label="Primary Navigation">
          <div className="max-w-7xl mx-auto flex justify-between items-center px-4 md:px-6 py-2 md:py-3">
            ...
            {/* Right controls: ONLY dark-mode toggle (no color changer) */}
            <div className="flex items-center">
              ...
              >
                {darkMode ? (
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 md:h-6 md:w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path d="M12 3v1m0 16v1m8.66-11.66l-.707.707M5.05 18.95l-.707.707m15.192 2.121l-.707-.707M5.05 5.05l-.707-.707M21 12h-1M4 12H3" /><circle cx="12" cy="12" r="5" stroke="currentColor" strokeWidth={2} /></svg>
                ) : (
                  <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 md:h-6 md:w-6" fill="currentColor" viewBox="0 0 24 24"><path d="M12 3a9 9 0 0 0 0 18 9 9 0 0 1 0-18z" /></svg>
                )}
              </button>
            </div>
          </div>
        ...
        <main className="max-w-7xl mx-auto px-4 md:px-6 pt-24 md:pt-28 space-y-28 md:space-y-48">
          ...
          {/* SKILLS (radial rings, polished & mobile-friendly) */}
          ...
          {/* PROJECTS (saffron accents, no images/badges) */}
