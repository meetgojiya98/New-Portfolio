...
/* ===========================================================
   OUT-OF-THIS-WORLD • HYPER ENHANCED PORTFOLIO (fixed + refined)
   - Fixes blank screen (corrupt JSX removed)
   - Removes right-side nav dots
   - Dynamic background (animated hue drift)
   - Project cards: no pictures/badges, clean meta
   =========================================================== */
...
const baseThemes = {
  saffron: { primary: "#f59e0b", primaryDark: "#d97706", primaryLight: "#fbbf24", particlesColor: "#fbbf24", ramps: ["#d97706", "#f59e0b", "#fde68a"] },
  blue:    { primary: "#3b82f6", primaryDark: "#2563eb", primaryLight: "#60a5fa", particlesColor: "#60a5fa", ramps: ["#2563eb", "#3b82f6", "#93c5fd"] },
  violet:  { primary: "#8b5cf6", primaryDark: "#7c3aed", primaryLight: "#a78bfa", particlesColor: "#a78bfa", ramps: ["#7c3aed", "#8b5cf6", "#c4b5fd"] },
  indigo:  { primary: "#6366f1", primaryDark: "#4f46e5", primaryLight: "#818cf8", particlesColor: "#818cf8", ramps: ["#4f46e5", "#6366f1", "#a5b4fc"] },
  fuchsia: { primary: "#d946ef", primaryDark: "#a21caf", primaryLight: "#e879f9", particlesColor: "#e879f9", ramps: ["#a21caf", "#d946ef", "#f5d0fe"] },
  rose:    { primary: "#f43f5e", primaryDark: "#e11d48", primaryLight: "#fb7185", particlesColor: "#fb7185", ramps: ["#e11d48", "#f43f5e", "#fecdd3"] },
  emerald: { primary: "#10b981", primaryDark: "#059669", primaryLight: "#34d399", particlesColor: "#34d399", ramps: ["#059669", "#10b981", "#6ee7b7"] },
  teal:    { primary: "#14b8a6", primaryDark: "#0d9488", primaryLight: "#2dd4bf", particlesColor: "#2dd4bf", ramps: ["#0d9488", "#14b8a6", "#99f6e4"] },
  cyan:    { primary: "#06b6d4", primaryDark: "#0891b2", primaryLight: "#22d3ee", particlesColor: "#22d3ee", ramps: ["#0891b2", "#06b6d4", "#67e8f9"] },
  lime:    { primary: "#84cc16", primaryDark: "#65a30d", primaryLight: "#a3e635", particlesColor: "#a3e635", ramps: ["#65a30d", "#84cc16", "#d9f99d"] },
  slate:   { primary: "#64748b", primaryDark: "#475569", primaryLight: "#94a3b8", particlesColor: "#94a3b8", ramps: ["#475569", "#64748b", "#cbd5e1"] },
};
...
function hexToHsl(hex) {
  ...
  const r = ((bigint >> 16) & 255) / 255, g = ((bigint >> 8) & 255) / 255, b = (bigint & 255) / 255;
...
const makeRamp = (h, s, l) => [
  hslToCss(h, s, clamp(l, 10, 90)),
  hslToCss(h, clamp(s + 5, 0, 100), clamp(l + 12, 0, 95)),
  hslToCss(h, clamp(s - 10, 0, 100), clamp(l + 28, 0, 98)),
];
...
// ------------------------------
// SHADERS • Nebula Plane (FBM) with dynamic hue drift, glow billboards
// ------------------------------
...
function Floating3DCanvas({ colors, reduced = false }) {
  ...
  return (
    <Canvas
      style={{ position: "fixed", inset: 0, pointerEvents: "auto", zIndex: 1, opacity: 0.92 }}
    ...
    >
      ...
      <Suspense fallback={null}>
        ...
        <InteractiveParticles color={primaryLight} count={reduced ? 80 : 120} />
...
function SectionReveal({ id, colors, title, children }) {
  ...
  return (
    <motion.section
      ...
      className="max-w-6xl mx-auto px-4 space-y-8 text-center relative z-10"
    >
...
export default function App() {
  const validThemes = ["blue","emerald","cyan","indigo","fuchsia","rose","teal","lime","saffron","violet","slate"];
      ...
      <div className={`relative bg-black/70 dark:bg-gray-950 text-gray-100 min-h-screen transition-colors duration-700 font-sans`} style={{ paddingBottom: "90px" }}>
        ...
        <nav className={`fixed top-0 w-full z-40 backdrop-blur-md bg-black/35 border-b border-white/10 transition-shadow ${scrolled ? "shadow-lg" : ""}`} role="navigation" aria-label="Primary Navigation">
          <div className="container mx-auto flex justify-between items-center px-6 py-3">
            ...
            {/* Right controls: ONLY dark-mode + theme cycle */}
            <div className="flex items-center space-x-3">
              <Magnetic>
                ...
                >
                  {darkMode ? (
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path d="M12 3v1m0 16v1m8.66-11.66l-.707.707M5.05 18.95l-.707.707m15.192 2.121l-.707-.707M5.05 5.05l-.707-.707M21 12h-1M4 12H3" /><circle cx="12" cy="12" r="5" stroke="currentColor" strokeWidth={2} /></svg>
                  ) : (
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="currentColor" viewBox="0 0 24 24"><path d="M12 3a9 9 0 0 0 0 18 9 9 0 0 1 0-18z" /></svg>
                  )}
                </button>
              </Magnetic>
              <Magnetic>
                <button
                  ...
                  onClick={() => { setUseCustom(false); const i = validThemes.indexOf(theme); const next = validThemes[(i + 1) % validThemes.length]; setTheme(next); }}
                  className="p-2 rounded-full bg-white/20 hover:bg-white/30 transition focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)] focus:ring-offset-2"
        ...
        {/* (Removed right-side minimap dots) */}
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-end justify-end p-6">
              <motion.div initial={{ x: 40, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: 40, opacity: 0 }} className="w/full max-w-sm rounded-2xl border border-white/10 bg-black/70 shadow-2xl overflow-hidden">
                ...
                <div className="p-4 space-y-4">
                  ...
                  <div><div className="text-sm mb-1">Hue: {h}°</div><input type="range" min="0" max="360" value={h} onChange={(e)=>setH(parseInt(e.target.value,10))} className="w-full" /></div>
                  <div><div className="text-sm mb-1">Saturation: {s}%</div><input type="range" min="0" max="100" value={s} onChange={(e)=>setS(parseInt(e.target.value,10))} className="w-full" /></div>
                  <div><div className="text-sm mb-1">Lightness: {l}%</div><input type="range" min="0" max="100" value={l} onChange={(e)=>setL(parseInt(e.target.value,10))} className="w-full" /></div>
                  <div className="flex gap-2 items-center">{makeRamp(h,s,l).map((c,i)=>(<div key={i} className="h-8 w-full rounded" style={{ background:c }} />))}</div>
        ...
        <main className="container mx-auto px-6 pt-24 space-y-48 max-w-7xl">
          ...
          <section id="hero" className="min-h-screen flex flex-col justify-center items-center text-center space-y-8 relative z-10">
            ...
            <motion.p initial={{ y: 60, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 1.1 }} className="max-w-2xl text-lg md:text-xl text-white/90">I craft resilient, data-driven products—blending gorgeous UI with grounded engineering. Dive into the work.</motion.p>
          ...
          <SectionReveal id="skills" colors={colors} title="Skills">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {[
                ...
                <div key={cat.title} className="glass-card rounded-2xl p-6 text-left">
                  <div className="mb-5 font-semibold tracking-wide text-lg" style={{ background: rampCSS, WebkitBackgroundClip: "text", color: "transparent" }}>{cat.title}</div>
          ...
          {/* PROJECTS (no images, no badge chips) */}
              ...
              {filteredProjects.map((p) => {
                ...
                return (
                  <TiltCard key={p.title} className="glass-card rounded-2xl overflow-hidden text-left group" glow={accent} onClick={() => setSelectedProject(p)}>
                    ...
                    {/* Editor-like chrome header (no image) */}
                    <div className="px-4 py-3 flex items-center justify-between border-b border-white/10">
                      <div className="flex gap-1">
                        <span className="w-2.5 h-2.5 rounded-full" style={{ background: accent }} />
                        <span className="w-2.5 h-2.5 rounded-full" style={{ background: accent2 }} />
                        <span className="w-2.5 h-2.5 rounded-full" style={{ background: colors.primaryLight }} />
                      </div>
                      <div className="text-xs uppercase tracking-widest text-white/60">{p.repo?.split("/")[1] || "project"}</div>
                    </div>
                    {/* Body */}
                    <div className="p-6 space-y-4">
                      <h3 className="text-xl font-semibold" style={{ background: accentRamp, WebkitBackgroundClip: "text", color: "transparent" }}>{p.title}</h3>
                      <p className="text-white/90">{p.description}</p>
                      <div className="text-xs uppercase tracking-wide text-white/60">{(p.tags || []).join(" • ")}</div>
            ...
            <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }} className="fixed inset-0 z-[60] bg-black/60 backdrop-blur-sm flex items-center justify-center p-6">
              <motion.div layout initial={{ scale:0.98, opacity:0 }} animate={{ scale:1, opacity:1 }} exit={{ scale:0.98, opacity:0 }} className="w-full max-w-3xl rounded-2xl overflow-hidden border border-white/10 bg-black/70">
                <div className="p-6 space-y-4">
                  ...
                  <div className="text-xs uppercase tracking-wide text-white/60">{(selectedProject.tags||[]).join(" • ")}</div>
                  <div className="flex gap-3">
                    ...
                    <div className="flex-1" />
                    <button onClick={()=>setSelectedProject(null)} className="px-4 py-2 rounded-lg border border-white/20 hover:bg-white/10">Close</button>
                  </div>
                </div>
              </motion.div>
            </motion.div>
        ...
        <a
          ...
          className="fixed bottom-20 right-6 z-50 text-black px-5 py-3 rounded-full shadow-lg transition flex items-center space-x-2 select-none focus:outline-none focus:ring-4 focus:ring-[var(--color-primary)] focus:ring-opacity-50"
        ...
        >
          <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M4 16v2a2 2 0 002 2h12a2 2 0 002-2v-2M12 12v8m0 0l-4-4m4 4l4-4M12 4v8" /></svg>
          <span>Resume</span>
        </a>
      </div>
    </>
