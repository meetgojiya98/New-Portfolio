...
function makeRamp(h,s,l){
  return [ hslToCss(h, s, clamp(l, 10, 90)), hslToCss(h, clamp(s+5, 0, 100), clamp(l+12, 0, 95)), hslToCss(h, clamp(s-10, 0, 100), clamp(l+28, 0, 98)) ];
}
...
function stringToHue(str){ let h=0; for(let i=0;i<str.length;i++){ h=(h*31 + str.charCodeAt(i))%360; } return h; }
  ...
  return (
    <mesh ref={meshRef} position={position} scale={scale} renderOrder={-1}>
      ...
      <shaderMaterial
        ...
        fragmentShader={`varying vec2 vUv; uniform vec3 uColor; uniform float uOpacity; void main(){ vec2 p=vUv-0.5; float d=length(p)*2.0; float a=smoothstep(1.0,0.0,d); a=pow(a,1.8); gl_FragColor=vec4(uColor,a*uOpacity); }`}
      />
    </mesh>
  );
}
...
function Floating3DCanvas({ colors }) {
  const { primary, primaryLight } = colors;
  return (
    <Canvas style={{ position: "fixed", inset: 0, pointerEvents: "auto", zIndex: 1, opacity: 0.85 }} camera={{ position: [0, 0, 12], fov: 65 }} gl={{ antialias: true, toneMapping: THREE.ACESFilmicToneMapping, toneMappingExposure: 1.25 }}>
...
function TiltCard({ children, className, max = 10, glow = "var(--color-primary)" }) {
  ...
  function onLeave() { const el = ref.current; if (!el) return; el.style.transform = "perspective(800px) rotateX(0) rotateY(0)"; el.style.boxShadow = "none"; }
...
function ScrollIndicator({ onClick }) {
  return (
    <motion.button onClick={onClick} initial={{ opacity: 0, y: 0 }} animate={{ opacity: 1, y: [0, 15, 0] }} transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }} className="absolute bottom-12 left-1/2 -translate-x-1/2 focus:outline-none" aria-label="Scroll down indicator" title="Scroll down" style={{ background: "transparent", border: "none", cursor: "pointer", padding: 0 }}>
      <svg className="w-8 h-8 text-gray-500 dark:text-gray-300" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7"/></svg>
    </motion.button>
  );
}
...
function SectionReveal({ id, colors, title, children }) {
  ...
  return (
    <motion.section id={id} ref={ref} initial={{ opacity: 0, y: 40 }} animate={inView ? { opacity: 1, y: 0, transition: { duration: 0.8 } } : undefined} className="max-w-5xl mx-auto px-4 space-y-8 text-center relative z-10">
      <motion.h2 className="text-4xl md:text-5xl font-extrabold tracking-tight" style={{ color: colors.primary }}>{title}</motion.h2>
...
function burstConfetti(x = window.innerWidth / 2, y = window.innerHeight / 2) {
  ...
  const EMOJIS = ["✨","🌟","💫","🪄","🔥","🧡","🎉","🌈"]; const pieces = 36;
  for (let i=0;i<pieces;i++) { const span=document.createElement("span"); span.textContent=EMOJIS[Math.floor(Math.random()*EMOJIS.length)]; span.style.position="absolute"; span.style.left=`${x}px`; span.style.top=`${y}px`; span.style.fontSize=`${Math.random()*18+14}px`; span.style.transition=`transform 800ms cubic-bezier(.2,.8,.2,1), opacity 900ms ease`; container.appendChild(span); const angle=(i/pieces)*Math.PI*2+Math.random()*0.4; const radius=120+Math.random()*120; const dx=Math.cos(angle)*radius; const dy=Math.sin(angle)*radius; requestAnimationFrame(()=>{ span.style.transform=`translate(${dx}px, ${dy}px) rotate(${Math.random()*360}deg)`; span.style.opacity="0"; }); }
...
export default function App() {
  ...
  const scrollTo = (id) => {
    ...
    window.scrollTo({ top: y, behavior: "smooth" }); setMenuOpen(false); setPaletteOpen(false);
  };
  ...
  const projects = useMemo(() => [
    {
      ...
      description: "End-to-end React and FastAPI app that delivers stock data, news, and AI insights for smarter investment decisions.",
    ...
    {
      title: "StockVision",
      description: "Responsive app that predicts future stock prices from real market data with interactive charts and favorites.",
    ...
    {
      title: "MapleLoom",
      description: "Precise, private RAG interface powered by Ollama, Qdrant, and Meilisearch. Offline. Source-anchored answers.",
  ...
  return (
    <>
      ...
      <div className={`relative bg-white dark:bg-gray-950 text-gray-900 dark:text-gray-100 min-h-screen transition-colors duration-700 font-sans`} style={{ paddingBottom: "90px" }}>
        ...
        <Floating3DCanvas colors={colors} />
        <AuroraLayer />
        <NoiseOverlay />
        ...
        <nav className={`fixed top-0 w-full z-40 backdrop-blur-md bg-white/70 dark:bg-gray-900/60 border-b border-gray-200 dark:border-gray-800 transition-shadow ${scrolled ? "shadow-lg" : ""}`} role="navigation" aria-label="Primary Navigation">
          <div className="container mx-auto flex justify-between items-center px-6 py-3">
            <div onClick={() => scrollTo("hero")} className="text-2xl font-extrabold cursor-pointer select-none tracking-tight" tabIndex={0} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") scrollTo("hero"); }} role="link" aria-label="Go to Home">Meet Gojiya</div>
            ...
            <div className="flex items-center space-x-3">
              ...
              <Magnetic>
                <button aria-label="Cycle Color Theme" onClick={() => setTheme(validThemes[(validThemes.indexOf(theme)+1)%validThemes.length])} className="p-2 rounded-full bg-gray-300 dark:bg-gray-800 hover:bg-gray-400 dark:hover:bg-gray-700 transition theme-cycle-btn focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)] focus:ring-offset-2" title="Cycle Color Theme">🎨</button>
              </Magnetic>
              <Magnetic>
                <button onClick={() => setPaletteOpen(true)} className="hidden md:inline-flex px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-700 hover:border-[var(--color-primary)]/80 hover:text-[var(--color-primary)] transition" title="Open command palette (⌘/Ctrl + K)">⌘K</button>
              </Magnetic>
              <Magnetic>
                <button onClick={() => setEditorOpen(true)} className="hidden md:inline-flex px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-700 hover:border-[var(--color-primary)]/80 hover:text-[var(--color-primary)] transition" title="Open theme editor (⌘/Ctrl + ,)">Theme</button>
              </Magnetic>
            </div>
          </div>
        </nav>
        ...
        <AnimatePresence>
          {editorOpen && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-end justify-end p-6">
              <motion.div initial={{ x: 40, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: 40, opacity: 0 }} className="w-full max-w-sm rounded-2xl border border-gray-200 dark:border-gray-800 bg-white/90 dark:bg-gray-900/90 shadow-2xl overflow-hidden">
                <div className="flex items-center justify-between px-4 py-3 border-b border-gray-200 dark:border-gray-800">
                  <div className="font-semibold">Theme Editor</div>
                  <button onClick={() => setEditorOpen(false)} className="px-2 py-1 rounded hover:bg-gray-200/50 dark:hover:bg-gray-800/50">✕</button>
                </div>
                <div className="p-4 space-y-4">
                  ...
                  <div className="flex justify-between pt-2">
                    <button className="px-3 py-2 rounded border border-gray-300 dark:border-gray-700 hover:bg-gray-100/60 dark:hover:bg-gray-800/60" onClick={()=>{ const b=baseThemes[theme]; const {h:hh,s:ss,l:ll}=hexToHsl(b.primary); setH(hh); setS(ss); setL(ll); }}>Reset to {theme}</button>
                    <button className="px-3 py-2 rounded" style={{ background: `linear-gradient(90deg, ${makeRamp(h,s,l).join(',')})`, color:'#000', boxShadow:'inset 0 0 0 1px rgba(0,0,0,.1)' }} onClick={()=>setUseCustom(true)}>Use These</button>
                  </div>
                </div>
              </motion.div>
            </motion.div>
        ...
        <main className="container mx-auto px-6 pt-24 space-y-48 max-w-6xl">
          ...
          <section id="hero" className="min-h-screen flex flex-col justify-center items-center text-center space-y-8 relative z-10">
            ...
            <motion.p initial={{ y: 60, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 1.5 }} className="max-w-xl text-lg md:text-xl">Full-stack Developer & AI Enthusiast — building beautiful, scalable web experiences.</motion.p>
            <Magnetic>
              <motion.button whileHover={{ scale: 1.07 }} whileTap={{ scale: 0.96 }} onClick={() => scrollTo("projects")} className="px-8 py-3 text-white rounded-xl shadow-lg transition z-10 focus:outline-none focus:ring-4 focus:ring-[var(--color-primary)] focus:ring-opacity-50" style={{ background: rampCSS }}>See My Work</motion.button>
            </Magnetic>
          ...
          <SectionReveal id="skills" colors={colors} title="Skills">
            <div className="overflow-hidden py-2">
              <div className="marquee">
                {["JavaScript (ES6+)","React.js & Next.js","Node.js & Express","MongoDB & SQL","Python & FastAPI","HTML5 & CSS3","Git & CI/CD","Pandas & NumPy","REST APIs","Postman & Jira","TypeScript","Java","AWS","OpenAI & AI Integration","Selenium", "WebGL", "LangChain" , "FastAPI"].concat(["JavaScript (ES6+)","React.js & Next.js","Node.js & Express","MongoDB & SQL","Python & FastAPI","HTML5 & CSS3","Git & CI/CD"]).map((skill, i) => (
                  <span key={i} className="px-5 py-2 rounded-full text-white font-semibold shadow-lg select-none" style={{ background: rampCSS }}>{skill}</span>
                ))}
              </div>
            </div>
          </SectionReveal>

          {/* PROJECTS (TILT CARDS GRID with THUMBS + accent) */}
          ...
          <SectionReveal id="contact" colors={colors} title="Contact Me">
            ...
            {contactStatus === "SUCCESS" && (<p className="mt-4 text-green-500 font-semibold" role="alert">Message sent successfully!</p>)}
            {contactStatus === "FAILED" && (<p className="mt-4 text-red-500 font-semibold" role="alert">Oops! Something went wrong. Please try again.</p>)}
          </SectionReveal>
        </main>
